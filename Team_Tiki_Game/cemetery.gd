...
func _ready() -> void:
	...
	if "gravestone" in Items:
		remove_child(gravestone)
		gravestone_button.text = "Gravestone"
	if "dress" in Items:
		dress_button.text = "Old Dress"
	if "cookie" in Items:
		cookie_button.text = "Homemade Cookie"
	gravestone.connect("gravestone_picked_up", Callable(self, "_on_gravestone_picked_up"))
...
func _on_gravestone_picked_up():
	$CutsceneHandler.SceneChanger(OBTAINED_GRAVESTONE)
